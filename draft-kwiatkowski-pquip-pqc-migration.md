...
Before initiating the transition to post-quantum cryptography (PQC), organizations should determine how to structure their migration strategy. In most cases, this involves starting migration activities immediately while adopting a phased approach that prioritizes systems based on risk and sensitivity. The overall strategy depends on factors such as the estimated duration of migration, the required confidentiality period of sensitive data, and the projected timeline for the emergence of cryptographically relevant quantum computers (CRQCs). Data with long-term confidentiality requirements may increase the urgency, as adversaries could collect encrypted information today and decrypt it in the future using a CRQC. Even in the absence of a direct harvest-now-decrypt-later threat, organizations must still prepare to protect data integrity and authenticity against future quantum-enabled attacks.
...
Long-lived verifiers - such as IoT devices deployed in the field and accepting software update - require protection against future quantum threats due to their extended operational lifetimes. As quantum computing capabilities progress, these verifiers must migrate to post-quantum cryptography (PQC) to preserve integrity and authenticity over time. For such use cases, hash-based signature schemes provide a practical near-term solution, offering strong post-quantum security and suitability for long-term validation scenarios. Additional guidance on integrating PQC into constrained environments—particularly relevant for long-lived verifiers—is available in {{Section 6 of ?I-D.ietf-pquip-pqc-hsm-constrained}}.

## PQ/T hybrid digital signatures

PQ/T hybrid digital signatures combine conventional and post-quantum algorithms to ensure long-term security against emerging quantum threats. This dual-layer approach mitigates the uncertainty surrounding newly standardized post-quantum schemes and their implementations — the traditional component provides proven, well-understood security, while the post-quantum component adds protection against future quantum attacks. However, this defense-in-depth strategy also introduces additional implementation and operational complexity, which must be carefully managed. Key considerations include:

- Hybrid signatures are larger in size and typically demand more computational effort, both of which can affect bandwidth and overall system performance. The two primary concerns are increased latency and higher resource consumption. The first is particularly critical for low-latency applications, while the second poses challenges for constrained environments, where computation time and memory usage are already limited and further stressed by the inherent complexity of post-quantum signature schemes.
- The ecosystem of supported PQ/T hybrid schemes is still evolving, and implementations and standards are continuing to mature. Protocols must be adapted to support hybrid signatures, which may require modifications to existing standards such as X.509 and their deployments (e.g., PKI).
- Systems supporting multiple algorithms also increase overall complexity, the potential for implementation errors, and the cost of certification (e.g., FIPS).
- The large number of potential hybrid combinations introduces interoperability challenges and increases implementation burden.
- The choice of hybrid combinations should be carefully considered to balance security, performance, and complexity.
- In case one of the schemes is compromised, the signature doesn't achieve strong unforgeability under chosen message attack - SUF-CMA (definition of SUF-CMA can be found in the Section 10.1.1 of [I-D.ietf-pquip-pqc-engineers]). This is a because, an attacker could forge a signature of the compromised scheme and exploit the structure of the PQ?T hybrid signature to create a valid, but different signature for the same message. However, we note that this limitation is generally acceptable in practice, as the primary goal of hybrid signatures is to provide defense-in-depth during the transition period to post-quantum cryptography (PQC). Namely it assumes that authenticity is provided by one of the signatures, but not necesairly both.
...
A key aspect of PQC migration involves identifying and budgeting for discovery initiatives, which aim to understand the organization's cryptographic assets and their PQC relevancy. Ensuring that an up-to-date cryptographic inventory is present will be required to identify outdated or vulnerable algorithms used by the IETF protocols. In some cases (i.e. relatively small infrastructure or one with limited use of cryptography), this may be a straightforward task and there may be no need for dedicated tooling.
